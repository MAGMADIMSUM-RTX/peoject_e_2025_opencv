# 切换到阶段2
                    ...
                    return dx, dy, "Phase1 Complete - Laser ON"
            alignment_status = "Phase1 Aligned"
        else:
            self.phase1_is_tracking = False
            self.phase1_track_cnt = 0
            alignment_status = None
        
        # 发送偏移数据到串口（阶段1使用视野中心对齐逻辑）
        print(f"Phase1 - 视野中心偏移: {-dx},{dy}")
        self.serial_controller.write(f"{-dx},{dy}\n")
        
        # 可选：读取串口返回的数据
        if self.serial_controller.in_waiting() > 0:
            response = self.serial_controller.read_line(timeout=0)
            if response:
                print(f"串口返回: {response}")
        
        self.phase1_last_dx = dx
        self.phase1_last_dy = dy
        
        return dx, dy, alignment_status
    
    def phase2_calculate_offset_and_check_alignment(self, laser_center_x, laser_center_y, target_center_x, target_center_y):
        """阶段2：计算激光光斑与A4中心的偏移量并检查对齐状态"""
        # 注意：这里dx和dy是A4中心点位置减激光光斑中心点位置的差
        dx = target_center_x - laser_center_x
        dy = target_center_y - laser_center_y
        
        # 检查对齐状态
        if abs(dx) + abs(dy) < config.ALIGNMENT_THRESHOLD and abs(self.phase2_last_dx) + abs(self.phase2_last_dy) < config.ALIGNMENT_THRESHOLD:
            if not self.phase2_is_tracking:
                self.phase2_track_cnt += 1
                if self.phase2_track_cnt > config.TRACK_COUNT_THRESHOLD:
                    self.phase2_is_tracking = True
                    self.phase2_track_cnt = 0
                    print("=== 阶段2对齐成功：激光光斑与A4中心对齐 ===")
            alignment_status = "Phase2 Aligned"
        else:
        ...
        self.phase2_last_dx = dx
        self.phase2_last_dy = dy
    ...
    def process_frame(self, frame):
        ...
        # 根据当前跟踪阶段执行不同的逻辑
        if self.tracking_phase == "PHASE1":
            # 阶段1：视野中心对齐A4中心（与basic2相同逻辑）
            ...
            # 计算偏移量并检查对齐
            dx, dy, alignment_status = self.phase1_calculate_offset_and_check_alignment(
                center_x, center_y, screen_center_x, screen_center_y
            )
            ...
            # 显示当前阶段
            cv2.putText(frame, f"PHASE 1: Align Screen Center to A4", 
                       (10, frame.shape[0] - 135), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
            
        elif self.tracking_phase == "PHASE2":
            # 阶段2：激光光斑对齐A4中心
            ...
            if self.laser_center is not None:
                # 计算激光光斑与A4中心的偏移量
                dx, dy, alignment_status = self.phase2_calculate_offset_and_check_alignment(
                    self.laser_center[0], self.laser_center[1], center_x, center_y
                )
            else:
                # 没有检测到激光光斑，发送搜索指令
                print("未检测到激光光斑，发送搜索指令")
                self.serial_controller.write("LASER:SEARCH\n")
                dx, dy, alignment_status = 0, 0, "Laser Lost"
            ...
            if laser_spot is not None:
                ...
                # 绘制A4中心与激光光斑的连线
                if self.laser_center is not None:
                    cv2.line(frame, (center_x, center_y), self.laser_center, (0, 255, 255), 2)
                    
                    # 显示追踪误差
                    error_distance = np.sqrt((center_x - self.laser_center[0])**2 + (center_y - self.laser_center[1])**2)
            ...
            info_y = self.display_manager.draw_distance_info(frame, distance_mm, avg_distance)
            # 在阶段2不显示screen center offset，而是显示laser-to-target offset
            cv2.putText(frame, f"Laser to Target Offset: ({dx}, {dy})", (10, info_y),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (100, 255, 100), 1)
            if alignment_status:
                cv2.putText(frame, alignment_status, (center_x + 20, center_y + 20), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
            
            self.display_manager.draw_status_info(frame, self.serial_controller.is_connected())
            
            # 显示当前阶段和稳定性信息
            cv2.putText(frame, f"PHASE 2: Align Laser to A4 Center", 
                       (10, frame.shape[0] - 135), 
    ...
    def run(self):
        ...
        print("跟踪阶段:")
        print("  阶段1: 视野中心对齐A4中心 (与basic2相同)")
        print("  阶段2: 激光光斑对齐A4中心 (激光开启后)")
    ...
    def show_final_statistics(self):
        ...
        if mean_distance is not None:
            ...
            print(f"最终跟踪阶段: {self.tracking_phase}")
            print(f"激光状态: {'开启' if self.laser_opened else '关闭'}")
...
def main():
    ...
    print("1. A4纸目标检测和跟踪")
    print("2. 两阶段跟踪策略:")
    print("   - 阶段1: 视野中心对齐A4中心（基于basic2逻辑）")
    print("   - 阶段2: 激光光斑对齐A4中心（激光开启后）")
    print("3. 蓝紫色激光光斑检测和稳定性跟踪")
    print("4. 智能位置预测和平滑处理")
    print("5. 云台控制指令发送")
    print("6. HMI指令接收和处理")
    print("7. 实时参数调节和调试功能")
